...
function parseArgs(args: string[]): { command: string; task?: string; maxSteps?: number; goalType?: GoalType; verbose?: boolean } {
  ...
  if (command === "run") {
    ...
    return { command, task, maxSteps, goalType, verbose };
  }
  return { command, maxSteps, goalType, verbose };
}
...
async function runTask(task: string, options: { maxSteps?: number; goalType?: GoalType; verbose?: boolean }): Promise<void> {
...
Usage:
  agent index
  agent run "Task description" [--max-steps N] [--goal-type TYPE] [--verbose]
...
async function main(): Promise<void> {
  ...
  const { command, task, maxSteps, goalType, verbose } = parseArgs(args);
  ...
  if (command === "run") {
    ...
    await runTask(task, { maxSteps, goalType, verbose });
    return;
  }
